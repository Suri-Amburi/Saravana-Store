...
*    DELETE lt_item_b WHERE material <> gv_matnr.
...
*** Updating in Delivery
  ...
  LOOP AT lt_item_data_f ASSIGNING FIELD-SYMBOL(<ls_data>).
*** Creation of HU
    ...
    READ TABLE lt_lips_m ASSIGNING FIELD-SYMBOL(<ls_lips>) WITH KEY vbeln = gv_del matnr = <ls_data>-material charg = ls_itemsproposal-batch pstyv = 'YNLN' .
***  For HU Qty
    IF sy-subrc = 0.
      ls_itemsproposal-pack_qty = <ls_data>-dlv_qty - <ls_lips>-lfimg.
    ELSE.
      ls_itemsproposal-pack_qty = <ls_data>-dlv_qty.
    ENDIF.
...
*** Get Delivery details after Batch Split
  ...
  LOOP AT lt_item_data_f ASSIGNING FIELD-SYMBOL(<ls_itemp>). "WITH KEY BATCH = WA_LIPS-CHARG.
...
***  Batch Item
      ...
      wa_vbpok-pikmg = <ls_itemp>-dlv_qty.
...
FORM clear_data .
  REFRESH : lt_lips_m.
  CLEAR : gv_matnr, gv_maktx, gv_charg, gv_b_count, gv_t_qty,gv_p_qty, gv_cur_field, gv_cur_value, lv_b_qty,gv_p_qty.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form GET_COUNT
*&---------------------------------------------------------------------*
