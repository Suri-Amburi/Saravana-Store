...
*    SET HANDLER LR_EVENT->HANDLE_USER_COMMAND  FOR GRID.
  ...
  REFRESH :LT_ITEM_SCR.
  LOOP AT LT_ITEM ASSIGNING <LS_ITEM>.
    MOVE-CORRESPONDING <LS_ITEM> TO LS_ITEM_SCR.
    LS_ITEM_SCR-SNO = SY-TABIX.
    APPEND LS_ITEM_SCR TO LT_ITEM_SCR.
  ENDLOOP.
...
***    No data Available for vendor 0000100001.
...
FORM CLEAR_ALL.
  IF OK_CODE = C_CLEAR.
    CLEAR : P_EBELN, P_QR_CODE,LV_MOD, WA_HDR, LV_TRNS, LV_STATUS,LV_ERROR ,
            LV_TAX_%,LV_TAX_CAT, LV_SOE_DES , LV_PROF%,LV_PROF_AMT, LV_GR_DATE, LV_GRC_PROF, LV_GRC_PROF%.
