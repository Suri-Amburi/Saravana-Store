...
FORM VALIDATE_PO.
  IF OK_CODE <> C_CLEAR.
    IF P_EBELN IS NOT INITIAL.
      SELECT SINGLE EBELN FROM EKKO INTO @DATA(LV_EBELN) WHERE EBELN = @P_EBELN.
      IF SY-SUBRC <> 0 .
...
*** Get Material Type : Branded or Non Branded
        SELECT MATNR BRAND_ID EANNR FROM MARA INTO TABLE LT_MARA FOR ALL ENTRIES IN LT_EKPO WHERE MATNR = LT_EKPO-MATNR AND BRAND_ID IS NOT NULL AND EANNR = C_UC  .
      ENDIF.
...
*** Skipping LR Number & Transporter Validation On Mandatory Fields for ZTAQ Doc type
    IF LV_BSART <> C_ZTAT.
      IF WA_HDR-TRNS IS INITIAL OR WA_HDR-LR_NO IS INITIAL OR WA_HDR-BILL_NUM IS INITIAL OR WA_HDR-BILL_DATE IS INITIAL AND WA_HDR-ACT_NO_BUD IS INITIAL .
        LV_ERROR = C_E.
    ...
    LOOP AT SCREEN.
      ...
      ELSE.
        ...
        ELSEIF WA_HDR-STATUS = '04' AND WA_HDR-SOE = '03'.
          IF SCREEN-NAME = 'B_SHORTAGE'.
            SCREEN-INPUT = 1.
            MODIFY SCREEN.
          ENDIF.
        ELSEIF WA_HDR-STATUS = '04' AND WA_HDR-SOE = '02'.
          IF SCREEN-NAME = 'B_EXCESS'.
            SCREEN-INPUT = 1.
            MODIFY SCREEN.
          ENDIF.
        ENDIF.
...
***     DOC TYPE CHECKING
  SELECT SINGLE BSART FROM EKKO INTO LV_BSART WHERE EBELN = P_EBELN.
...
*&---------------------------------------------------------------------*
FORM F4_TRANS .
*  SELECT LIFNR , NAME1 INTO TABLE @DATA(IT_TRNS) FROM LFA1 WHERE KTOKK = 'KRED'.
*  IF IT_TRNS IS NOT INITIAL.
...
FORM CLEAR_ALL.
  IF OK_CODE = C_CLEAR.
    CLEAR : P_EBELN, P_QR_CODE,LV_MOD, WA_HDR, LV_TRNS, LV_STATUS,LV_ERROR , LV_TAX_%,LV_TAX_CAT, LV_SOE_DES.
...
*    EXPORTING
*      TITLEBAR       = 'CANCEL'
*      TEXT_QUESTION  = 'Once the QR code document complited you can not do further changes Click ok to Continue'
*      TEXT_BUTTON_1  = 'Yes'
*      TEXT_BUTTON_2  = 'No'
*      DEFAULT_BUTTON = '1'
*    IMPORTING
