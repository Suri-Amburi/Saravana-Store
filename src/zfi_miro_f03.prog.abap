...
*          READ TABLE GT_TAX_CODE ASSIGNING FIELD-SYMBOL(<LS_TAX_CODE>) WITH KEY MWSKZ = <LS_MATDOC_107>-MWSKZ.
*          IF SY-SUBRC = 0.
*            LOOP AT GT_KONP ASSIGNING FIELD-SYMBOL(<LS_KONP>) WHERE KNUMH = <LS_TAX_CODE>-KNUMH.
*              LV_TAX = ( <LS_KONP>-KBETR * LS_ITEMDATA-ITEM_AMOUNT  ) / 1000.
*              ADD LV_TAX TO LV_TAX_TOL.
*            ENDLOOP.
*          ENDIF.

**** Tax Calculation Item Wise
*          ZCL_PO_ITEM_TAX=>GET_PO_ITEM_TAX(
*            EXPORTING
*              I_EBELN     =  LS_ITEMDATA-PO_NUMBER                " Purchasing Document Number
*              I_EBELP     =  LS_ITEMDATA-PO_ITEM                  " Item Number of Purchasing Document
*              I_QUANTITY  =  LS_ITEMDATA-QUANTITY                 " Quantity
*            IMPORTING
*              E_TAX       =  DATA(LV_TAX_I)                       " Tax Amount in Document Currency
*              E_TOTAL_VAL =  DATA(LV_TOTAL_I)                     " Net Value in Document Currency
*          ).

*          HEADERDATA-GROSS_AMOUNT = HEADERDATA-GROSS_AMOUNT + LV_TOTAL_I.
...
*        CLEAR : LV_TAX_TOL.
*        READ TABLE GT_TAX_CODE ASSIGNING <LS_TAX_CODE> WITH KEY MWSKZ = <LS_MATDOC>-MWSKZ.
*        IF SY-SUBRC = 0.
*          LOOP AT GT_KONP ASSIGNING <LS_KONP> WHERE KNUMH = <LS_TAX_CODE>-KNUMH.
*            LV_TAX = ( <LS_KONP>-KBETR * LS_ITEMDATA-ITEM_AMOUNT  ) / 1000.
*            ADD LV_TAX TO LV_TAX_TOL.
*          ENDLOOP.
*        ENDIF.
*        HEADERDATA-GROSS_AMOUNT = HEADERDATA-GROSS_AMOUNT + LS_ITEMDATA-ITEM_AMOUNT + LV_TAX.

**** Tax Calculation Item Wise - Qantity Wise
*        ZCL_PO_ITEM_TAX=>GET_PO_ITEM_TAX(
*          EXPORTING
*            I_EBELN     =  LS_ITEMDATA-PO_NUMBER                " Purchasing Document Number
*            I_EBELP     =  LS_ITEMDATA-PO_ITEM                  " Item Number of Purchasing Document
*            I_QUANTITY  =  LS_ITEMDATA-QUANTITY                 " Quantity
*          IMPORTING
*            E_TAX       =  LV_TAX_I                             " Tax Amount in Document Currency
*            E_TOTAL_VAL =  LV_TOTAL_I                           " Net Value in Document Currency
*        ).
*        HEADERDATA-GROSS_AMOUNT = HEADERDATA-GROSS_AMOUNT + LV_TOTAL_I.
        LV_DOC_ITEM = LV_DOC_ITEM + 1.
        APPEND LS_ITEMDATA TO ITEMDATA.
...
*        LV_TAX_AMOUNT = LS_ITEMDATA-ITEM_AMOUNT + ( ( LS_ITEMDATA-ITEM_AMOUNT * <LS_DEBIT>-KBETR ) / 1000 ) .
*        LV_TAX_AMOUNT = LV_TAX_AMOUNT + LV_TAX_AMOUNT.
*      ENDIF.
