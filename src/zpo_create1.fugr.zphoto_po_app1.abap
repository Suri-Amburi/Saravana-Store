...
***  Header Data
      ...
      wa_ph_hdr-freight_charges = wa_header-freight_charges.
      wa_ph_hdr-category_type   = wa_header-category_type.
...
*** Vendor
      ...
      wa_ph_hdr-vendor    = lv_lifnr.
      wa_ph_hdr-vendor_name     = ls_vendor-name1.
...
*** Get Groupwise Discount
      ...
      bapi_te_po-zzcategory_type  = c_e.
      ...
      bapi_te_pox-zzcategory_type  = c_x.
...
*** HSN & Tax Code
      ...
      IF wa_header-pur_group = 'P34'.
        ...
        IF lv_eknam IS NOT INITIAL.
          SELECT SINGLE gl_account,costcenter FROM zgl_acc_t INTO @DATA(ls_data) WHERE wwgha = @lv_eknam AND werks = @wa_header-delivery_at.
        ENDIF.
      ENDIF.

*** Freight charges
      APPEND VALUE #( cond_type = c_zfrb cond_value = wa_header-freight_charges / 10 change_id = 'I' ) TO pocondhdr[] .
...
*** Item Data
      ...
      IF ph_item[] IS NOT INITIAL.
        LOOP AT ph_item ASSIGNING FIELD-SYMBOL(<ls_ph_item>).
          ...
          ls_ph_item-indent_no     = wa_header-indent_no.
          ls_ph_item-item          = lv_count.
          ls_ph_item-vendor        = lv_lifnr.
          ls_ph_item-matnr         = <ls_ph_item>-matnr.
          ls_ph_item-pgroup        = wa_header-pur_group.
          ls_ph_item-category_code = <ls_ph_item>-category_code.
          ls_ph_item-style         = <ls_ph_item>-style.
          ls_ph_item-from_size     = <ls_ph_item>-from_size.
          ls_ph_item-to_size       = <ls_ph_item>-to_size.
          ls_ph_item-color         = <ls_ph_item>-color.
          ls_ph_item-quantity      = <ls_ph_item>-quantity.
          ls_ph_item-price         = <ls_ph_item>-price.
          ls_ph_item-remarks       = <ls_ph_item>-remarks.
          ls_ph_item-ztext100      = lv_count && <ls_ph_item>-matnr && <ls_ph_item>-style && <ls_ph_item>-from_size && <ls_ph_item>-to_size  && <ls_ph_item>-color && <ls_ph_item>-price.
          ls_ph_item-discount2     = <ls_ph_item>-discount2.
          ls_ph_item-discount3     = <ls_ph_item>-discount3.
...
*** Consumbales - to Cost Center
            IF wa_header-pur_group = 'P34'.
              ...
              APPEND VALUE #( po_item  = lv_count gl_account = c_x costcenter = c_x ) TO poaccountx.
            ENDIF.
...
*** Discount 3 per Piece
            APPEND VALUE #( cond_type = c_zds3 cond_value = <ls_ph_item>-discount3 / 10 itm_number = lv_count change_id = 'I' ) TO pocond[] .
            APPEND VALUE #( cond_type = c_x cond_value = c_x itm_number = c_x change_id = c_x ) TO pocondx[] .
*
**** Freight charges
...
*** End Of Changes by Suri : For Group/ Vendor level Discount : 23.03.2020 : 11.11.00
          ELSE.
*** No Tax Code Found
            ls_ph_item-e_msg = 'No tax Code'.
            APPEND INITIAL LINE TO et_return ASSIGNING FIELD-SYMBOL(<ls_return>).
            <ls_return>-type = 'E'.
            <ls_return>-message = ls_ph_item-e_msg && ' For line item : ' && lv_count.
            lv_error = c_e.
          ENDIF.
          APPEND ls_ph_item TO it_ph_item.
        ENDLOOP.

        MODIFY zph_t_hdr  FROM wa_ph_hdr.
        MODIFY zph_t_item FROM TABLE it_ph_item.
        Commit work.
        ...
        IF lv_error IS INITIAL.
          ...
          et_return[] = return[].
          READ TABLE return ASSIGNING <ls_return> WITH KEY type = c_e.
          IF sy-subrc IS NOT INITIAL AND lv_ebeln IS NOT INITIAL.
            CALL FUNCTION 'BAPI_TRANSACTION_COMMIT' EXPORTING wait = 'X'.
            header_return-success_message = lv_ebeln && ' Successfully Created'.
          ...
          ELSE.
            ...
            header_return-error_message = 'Error'.
          ENDIF.
        ENDIF.
      ELSE.
