*&---------------------------------------------------------------------*
*& Include          SAPMZMM_SUBCON_SALES_CLS
*&---------------------------------------------------------------------*
...
*      PERFORM get_matdtls .
      ...
      READ TABLE xsubcon_itm INTO DATA(temp) INDEX entr_data-row_id.
...
*
        temp-req_qty = xsubcon_hdr-menge * entrd_menge.

        IF temp-req_qty > temp-clabs.
          temp-icon = sred.
          smsg  = temp-clabs - temp-req_qty.
...
*             i_msgv3     = 'E' "exitstiert nicht
        ...
        ELSE.
          temp-icon = sgreen.
          MODIFY xsubcon_itm FROM temp INDEX entr_data-row_id TRANSPORTING icon req_qty.
          PERFORM scontainer .
        ENDIF.
      ELSE.
      ENDIF.
