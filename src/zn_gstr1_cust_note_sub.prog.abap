*&---------------------------------------------------------------------*
*& Include          ZN_GSTR1_CUST_NOTE_SUB
*&---------------------------------------------------------------------*
...
FORM select_querry .
  ...
  SELECT  bukrs
          ...
          WHERE budat IN s_budat AND gjahr IN pyear AND bukrs IN s_bukrs AND blart = 'DR'
          AND  XREVERSED NE 'X'
          AND  XREVERSING NE 'X' .
...
*            AND BSCHL = 11
    ...
    IF it_bseg IS NOT INITIAL.
      ...
      SELECT vbeln
             ...
             kunag
             waerk
             zuonr FROM vbrk INTO TABLE it_vbrk
                   FOR ALL ENTRIES IN it_bseg
      ...
      SELECT vbeln
             ...
             kunag
             waerk
             zuonr FROM vbrk INTO TABLE it_vbrk1
...
*      LV_SLN = LV_SLN + 1.
*      WA_FIN-SLNO = LV_SLN.
*    WA_FIN-ZPOFS = '10'.   "Place of Supply
...
*          wa_fin-totval  = wa_fin-totval + wa_bset-hwbas * ( -1 ) .
...
*        ENDLOOP.
        ENDLOOP.
         lv_sln = lv_sln + 1.
         wa_fin-slno = lv_sln.
         wa_fin-totval = wa_fin-dmbtr - ( wa_fin-sgst + wa_fin-cgst + wa_fin-igst + wa_fin-ugst + wa_fin-cess ) .
         wa_fin-dmbtr = wa_fin-totval + ( wa_fin-sgst + wa_fin-cgst + wa_fin-igst + wa_fin-ugst + wa_fin-cess ) .

         APPEND wa_fin TO it_fin.
         CLEAR : wa_fin-totval ,wa_fin-sgst , wa_fin-cgst , wa_fin-igst , wa_fin-ugst , wa_fin-cess ,wa_fin-dmbtr.
      ELSE.
...
*          wa_fin-totval  =  wa_fin-totval + wa_bset-hwbas .
          IF  wa_bset-kschl    = 'JOSG' OR wa_bset-kschl = 'JISG'.
*            if lv_taxcode =  wa_bset-mwskz.
...
*        lv_name1 TYPE ad_name1,
  ...
  lv_top = 'GSTR1-Customer Accounting Invoice'.
