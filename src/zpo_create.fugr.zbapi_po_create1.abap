...
*"*"Local Interface:
*"  IMPORTING
*"     VALUE(IM_HEADER_TT) TYPE  ZPOHEADERTT
*"  EXPORTING
...
*         WA_A792    TYPE TY_A792.
  ...
  DATA : IM_HEADER TYPE  ZPOHEADER.
  DATA : LV_POITEM TYPE EBELP,
         LV_ERNAME TYPE ERNAM.

*** Start Of Changes By Suri : 11.05.2019
  CONSTANTS :
    C_SET(3)        VALUE 'SET',   " SET UOM
    C_M(1)          VALUE 'M',     " BOM category
    C_X(1)          VALUE 'X',     " X
    C_INTERFACE(10) VALUE 'INTERFACE',
...
*** End Of Changes By Suri : 11.05.2019
*  BREAK BREDDY.
  ...
  IF SY-SUBRC = 0 .
    ...
    HEADER-COMP_CODE = IM_HEADER-BUKRS.
    HEADERX-COMP_CODE = 'X'.
    ...
    HEADER-LANGU = 'X' .
    HEADER-PURCH_ORG = IM_HEADER-EKORG .
    HEADERX-PURCH_ORG = 'X'.
...
*    BREAK BREDDY.
    ...
    READ TABLE PO_ITEM ASSIGNING FIELD-SYMBOL(<LS_ITEM>) WITH KEY MEINS = C_SET.
    IF SY-SUBRC = 0.
      SELECT MAST~MATNR,
         MAST~WERKS,
         MAST~STLNR,
         ...
         WHERE MAST~MATNR = @PO_ITEM-MATNR.
    ENDIF.
    BREAK BREDDY.
...
********added by bhavani 30.09.2019**************
    SELECT
    MARA~MATNR ,
    MARA~SATNR FROM MARA INTO TABLE  @DATA(IT_SATNR)
    FOR ALL ENTRIES IN @PO_ITEM
...
***added by bhavani 22.10.2019 for HSN Codes***
    SELECT
      MARC~MATNR ,
      MARC~STEUC  FROM MARC INTO TABLE @DATA(IT_MARC)
                 FOR ALL ENTRIES IN @PO_ITEM
                 WHERE MATNR = @PO_ITEM-MATNR .


*    SELECT SINGLE
*      T001W~REGIO FROM T001W INTO @DATA(LV_PLANT)
*                  WHERE WERKS = @PO_ITEM-WERKS .
    ...
    SELECT
     ...
     T001W~WERKS
      ...
      FOR ALL ENTRIES IN @PO_ITEM
      WHERE MARC~MATNR = @PO_ITEM-MATNR
      AND A792~REGIO   = @LS_LFA1
      AND T001W~WERKS = @PO_ITEM-WERKS.
...
*** Start Of Changes By Suri : 11.05.2019
*** Checking For SET Material
      IF WA_PO_ITEM-MEINS = C_SET.


********start changes by bhavani 22.10.2019****
        READ TABLE IT_HSN ASSIGNING FIELD-SYMBOL(<LS_HSN>) WITH KEY MATNR = WA_PO_ITEM-MATNR .
        IF SY-SUBRC = 0.
          READ TABLE IT_KONP ASSIGNING FIELD-SYMBOL(<LS_KONP>) WITH KEY KNUMH = <LS_HSN>-KNUMH .
          IF SY-SUBRC = 0.

            WA_ITEM-TAX_CODE = <LS_KONP>-MWSK1.
            LV_MWSKZ = <LS_KONP>-MWSK1 .
            WA_ITEMX-TAX_CODE = 'X'.
...
********ended by bhavani************************
        ...
        LOOP AT LT_COMP ASSIGNING FIELD-SYMBOL(<LS_COMP>) WHERE MATNR = WA_PO_ITEM-MATNR.
***       Itam Data
***       QUANTITY DIVISION
          DATA(LT_COMP_T) = LT_COMP.
          DELETE LT_COMP_T WHERE MATNR <> WA_PO_ITEM-MATNR.
          DESCRIBE TABLE LT_COMP_T LINES DATA(LV_LINES).
          LV_POITEM = LV_POITEM + 10.
          WA_ITEM-PO_ITEM = WA_ITEMX-PO_ITEM = WA_PO_ITEM-EBELP = LV_POITEM.
          SHIFT WA_PO_ITEM-MATNR LEFT DELETING LEADING '0'.
********added by bhavani 26.09.2019 ***************
*          WA_ITEM-MATERIAL      = <LS_COMP>-IDNRK.
*          WA_ITEM-MATERIAL_EXTERNAL      = <LS_COMP>-IDNRK.
...
********ended by bhavani 26.09.2019 ***************
          WA_ITEM-QUANTITY      = ( WA_PO_ITEM-MENGE * <LS_COMP>-MENGE ) / LV_LINES.
          WA_ITEM-PO_UNIT       = <LS_COMP>-MEINS.
          WA_ITEM-PLANT         = WA_PO_ITEM-WERKS.
          WA_ITEM-STGE_LOC      = WA_PO_ITEM-LGORT.
*          WA_ITEM-NET_PRICE     = WA_PO_ITEM-NETPR / LV_LINES.
...
***      Update Flags
          ...
          WA_ITEMX-PRICE_UNIT   = C_X.
          WA_ITEMX-PLAN_DEL     = C_X.
          WA_ITEMX-IR_IND       = C_X.
          WA_ITEMX-GR_BASEDIV   = C_X.
***       PO Text
...
*          WA_ITEMX-TAX_CODE = 'X'.
          ...
          LOOP AT IT_TAX INTO WA_TAX.
            ...
            IBAPICONDX-COND_VALUE  = C_X.
            IBAPICONDX-CURRENCY    = C_X.
            IBAPICONDX-CHANGE_ID   = C_X.
            APPEND IBAPICOND.
            APPEND IBAPICONDX.
            CLEAR : IBAPICOND, IBAPICONDX.
          ENDLOOP.
          APPEND WA_ITEM TO ITEM[].
          APPEND WA_ITEMX TO ITEMX[].

***       Header Extenction Fields
...
***       Header Extenction Fields update Flag
          WA_EXTENSIONIN-STRUCTURE  = 'BAPI_TE_MEPOHEADERX'.
          BAPI_TE_POX-PO_NUMBER     = ' '.
          BAPI_TE_POX-AGENT_NAME    = C_X.
          BAPI_TE_POX-USER_NAME     = C_X.
          BAPI_TE_POX-ERDATE        = C_X.
          BAPI_TE_POX-APPROVER1     = C_X.
          BAPI_TE_POX-APPROVER1_DT  = C_X.
          BAPI_TE_POX-APPROVER2     = C_X.
          BAPI_TE_POX-APPROVER2_DT  = C_X.
...
***       Item extenction fields
          ...
          BAPI_TE_POITEM-PO_ITEM  = WA_ITEM-PO_ITEM.
          BAPI_TE_POITEM-ZZSET_MATERIAL  = WA_PO_ITEM-MATNR.
          WA_EXTENSIONIN-VALUEPART1 = BAPI_TE_POITEM.
          APPEND WA_EXTENSIONIN TO EXTENSIONIN.
          CLEAR : WA_EXTENSIONIN.
***       Item extenction fields Updation Flags
          WA_EXTENSIONIN-STRUCTURE = 'BAPI_TE_MEPOITEMX'.
          BAPI_TE_POITEMX-PO_ITEM = WA_ITEM-PO_ITEM.
          BAPI_TE_POITEMX-ZZSET_MATERIAL = C_X.
...
********start changes by bhavani 22.10.2019****
        READ TABLE IT_HSN ASSIGNING FIELD-SYMBOL(<LS_HSN1>) WITH KEY MATNR = WA_PO_ITEM-MATNR .
        IF SY-SUBRC = 0.
...
*******start of changes by bhavani 26.09.2019 ***********
*        WA_ITEM-MATERIAL = WA_PO_ITEM-MATNR.
*        WA_ITEM-MATERIAL_EXTERNAL  = WA_PO_ITEM-MATNR.
        WA_ITEM-MATERIAL_LONG  = WA_PO_ITEM-MATNR.
*        WA_ITEMX-MATERIAL = 'X'.
*        WA_ITEMX-MATERIAL_EXTERNAL  = 'X'.
        WA_ITEMX-MATERIAL_LONG  = 'X'.
************end of changes by bhavani 26.09.2019**********
...
***   Start of changes by Suri : 12.09.2019
***   For Vessels Converstion KGs to EA
        IF IM_HEADER-GROUP_ID = C_VESSELS.
          WA_ITEM-ORDERPR_UN  = C_KG.
          WA_ITEMX-ORDERPR_UN = C_X.
        ENDIF.
***   End of changes by Suri : 12.09.2019
...
*    WA_POSERVICESTEXT-TEXT_ID = 'F03'.
*    WA_POSERVICESTEXT-FORMAT_COL = '*'.
***********added by bhavani 09.30.2019************

        CLEAR :BAPI_TE_POITEM ,BAPI_TE_POITEMX.
***       Item extenction fields
        WA_EXTENSIONIN-STRUCTURE = 'BAPI_TE_MEPOITEM'.
        BAPI_TE_POITEM-PO_ITEM  = WA_ITEM-PO_ITEM.
        BAPI_TE_POITEM-ZZCOLOR  = WA_PO_ITEM-COLOR.
        WA_EXTENSIONIN-VALUEPART1 = BAPI_TE_POITEM.
        APPEND WA_EXTENSIONIN TO EXTENSIONIN.
        CLEAR : WA_EXTENSIONIN.
***       Item extenction fields Updation Flags
...
***       Item extenction fields
        WA_EXTENSIONIN-STRUCTURE = 'BAPI_TE_MEPOITEM'.
        BAPI_TE_POITEM-PO_ITEM  = WA_ITEM-PO_ITEM.
        BAPI_TE_POITEM-ZZSTYLE  = WA_PO_ITEM-STYLE.
        WA_EXTENSIONIN-VALUEPART1 = BAPI_TE_POITEM.
        APPEND WA_EXTENSIONIN TO EXTENSIONIN.
        CLEAR : WA_EXTENSIONIN.
***       Item extenction fields Updation Flags
...
***       Item extenction fields
        WA_EXTENSIONIN-STRUCTURE = 'BAPI_TE_MEPOITEM'.
        BAPI_TE_POITEM-PO_ITEM  = WA_ITEM-PO_ITEM.
        BAPI_TE_POITEM-ZZREMARKS  = WA_PO_ITEM-REMARKS.
        WA_EXTENSIONIN-VALUEPART1 = BAPI_TE_POITEM.
        APPEND WA_EXTENSIONIN TO EXTENSIONIN.
        CLEAR : WA_EXTENSIONIN.
***       Item extenction fields Updation Flags
...
*        SELECT SINGLE
*         MARA~SATNR FROM MARA INTO @DATA(LV_SATNR)
*         WHERE MATNR = @WA_PO_ITEM-MATNR.


        BREAK BREDDY .
        IF  IT_SATNR IS NOT INITIAL .
          READ TABLE IT_SATNR ASSIGNING FIELD-SYMBOL(<LS_SATNR>) WITH  KEY MATNR = WA_PO_ITEM-MATNR .
***       Item extenction fields
          WA_EXTENSIONIN-STRUCTURE = 'BAPI_TE_MEPOITEM'.
          BAPI_TE_POITEM-PO_ITEM  = WA_ITEM-PO_ITEM.
          BAPI_TE_POITEM-ZZTEXT100  = <LS_SATNR>-SATNR && WA_PO_ITEM-COLOR && WA_PO_ITEM-STYLE.
          WA_EXTENSIONIN-VALUEPART1 = BAPI_TE_POITEM.
          APPEND WA_EXTENSIONIN TO EXTENSIONIN.
          CLEAR : WA_EXTENSIONIN.
***       Item extenction fields Updation Flags
...
**** Start of changes by Suri : 20.5.2019 : For Tracking the Receiving data from APP
*    DATA : LT_ITEM TYPE TABLE OF ZPOITEM_T,
*           LS_ITEM TYPE ZPOITEM_T.
*    REFRESH : LT_ITEM.
*    IF SY-UNAME  = C_INTERFACE.
*      LOOP AT PO_ITEM ASSIGNING FIELD-SYMBOL(<LS_ITEM_DB>).
*        MOVE-CORRESPONDING <LS_ITEM_DB> TO LS_ITEM.
*        LS_ITEM-EBELN = LV_EBELN.
*        LS_ITEM-EBELP = SY-TABIX * 10.
*        LS_ITEM-DATE_RECEIVED = SY-DATUM.
*        LS_ITEM-TIME_RECEIVED = SY-UZEIT.
*        APPEND LS_ITEM TO LT_ITEM.
*      ENDLOOP.
*      IF LT_ITEM IS NOT INITIAL.
*        MODIFY ZPOITEM_T FROM TABLE LT_ITEM.
*      ENDIF.
*    ENDIF.
**** End of changes by Suri : 20.5.2019
*
...
**           I_ADDRESS_STRING        TYPE TVARVC-LOW,
*           ES_MSG(100)             TYPE C.
*    DATA : LV_A       TYPE C,
*           LV_B       TYPE C,
*           LV_C       TYPE C,
...
**    ENDIF.
*    BREAK BREDDY.
*    SELECT MARA~MATNR  MARA~MATKL  MARA~ZZPO_ORDER_TXT  MARA~SIZE1 MARA~COLOR  MARA~EAN11 FROM MARA INTO CORRESPONDING FIELDS OF TABLE IT_MARA FOR ALL ENTRIES IN IT_EKPO WHERE MATNR = IT_EKPO-MATNR .
...
******************************If material is set*******************
**    BREAK BREDDY.
*    LV_HEADING = 'PURCHASE ORDER FORM'.
*    P_AEDAT  = SY-DATUM .
*
*    BREAK BREDDY.
...
*          WA_POITEM-MENGE = WA_EKPO_SET-MENGE * LV_LINES_SET .
*          WA_POITEM-MT_GRP = WA_EKPO_SET-MATKL.
*          LV_POITEM = LV_POITEM + 10.
*          WA_POITEM-EBELP = LV_POITEM.
*          WA_POITEM-NETPR = WA_EKPO_SET-NETPR.
...
*endif.
    ELSE .
      ...
      MESSAGE 'Tax code is not maintained' TYPE 'E' .
    ENDIF.
  ENDIF.
  CLEAR : LV_MWSKZ .
  IF LV_EBELN IS NOT INITIAL.
...
*  APPEND VALUE #( TYPE = 'E' NUMBER = '001'  MESSAGE = 'Contact SAP Administration ' ) TO ET_RETURN.
