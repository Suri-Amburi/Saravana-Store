class ZCL_ZGW_VENDOR_MPC definition
  public
...
public section.
  ...
  types:
     TS_VENDOR_TYPE type ZGW_VENDOR .
  types:
TT_VENDOR_TYPE type standard table of TS_VENDOR_TYPE .
  types:
  ...
  constants GC_VENDOR_TYPE type /IWBEP/IF_MGW_MED_ODATA_TYPES=>TY_E_MED_ENTITY_NAME value 'vendor_type' ##NO_TEXT.
...
private section.
  ...
  constants GC_INCL_NAME type STRING value 'ZCL_ZGW_VENDOR_MPC============CP' ##NO_TEXT.

  methods DEFINE_VENDOR_TYPE
    raising
      /IWBEP/CX_MGW_MED_EXCEPTION .
ENDCLASS.
...
CLASS ZCL_ZGW_VENDOR_MPC IMPLEMENTATION.
...
model->set_schema_namespace( 'ZGW_VENDOR_SRV' ).

define_vendor_type( ).
  endmethod.


  method DEFINE_VENDOR_TYPE.
*&---------------------------------------------------------------------*
*&           Generated code for the MODEL PROVIDER BASE CLASS         &*
*&                                                                     &*
*&  !!!NEVER MODIFY THIS CLASS. IN CASE YOU WANT TO CHANGE THE MODEL  &*
...
lo_entity_type = model->create_entity_type( iv_entity_type_name = 'vendor_type' iv_def_entity_set = abap_false ). "#EC NOTEXT
...
lo_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( 'sap' )->add(
      EXPORTING
        iv_key      = 'unicode'
        iv_value    = 'false' ).
lo_property = lo_entity_type->create_property( iv_property_name = 'Addr2Street' iv_abap_fieldname = 'ADDR2_STREET' ). "#EC NOTEXT
lo_property->set_type_edm_string( ).
...
lo_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( 'sap' )->add(
      EXPORTING
        iv_key      = 'unicode'
        iv_value    = 'false' ).
lo_property = lo_entity_type->create_property( iv_property_name = 'Addr2HouseNum' iv_abap_fieldname = 'ADDR2_HOUSE_NUM' ). "#EC NOTEXT
lo_property->set_type_edm_string( ).
lo_property->set_maxlength( iv_max_length = 10 ). "#EC NOTEXT
lo_property->set_creatable( abap_false ).
lo_property->set_updatable( abap_false ).
...
lo_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( 'sap' )->add(
      EXPORTING
        iv_key      = 'unicode'
        iv_value    = 'false' ).
lo_property = lo_entity_type->create_property( iv_property_name = 'LocalOut' iv_abap_fieldname = 'LOCAL_OUT' ). "#EC NOTEXT
...
lo_entity_type->bind_structure( iv_structure_name   = 'ZGW_VENDOR'
...
*   ENTITY SETS
***********************************************************************************************************************************
lo_entity_set = lo_entity_type->create_entity_set( 'vendor_typeSet' ). "#EC NOTEXT
...
*&---------------------------------------------------------------------*
  ...
  CONSTANTS: lc_gen_date_time TYPE timestamp VALUE '20190902220849'.                  "#EC NOTEXT
...
clear ls_text_element.
ls_text_element-artifact_name          = 'LocalOut'.                 "#EC NOTEXT
ls_text_element-artifact_type          = 'PROP'.                                       "#EC NOTEXT
ls_text_element-parent_artifact_name   = 'vendor_type'.                            "#EC NOTEXT
