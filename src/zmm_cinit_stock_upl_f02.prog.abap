...
***  FM TO UPLOAD DATA INTO INTERNAL TABLE FROM EXCEL
  ...
  ELSE.
****    LOOP AT gt_file INTO DATA(gw_data).
****      MOVE-CORRESPONDING gw_data TO gw_file01.
****      gw_data-uname = sy-uname. "User Name
****      gw_data-erdat = sy-datum. "DATE
****      APPEND gw_file01 TO gt_file01.
****      CLEAR : gw_file01 .
****    ENDLOOP.
***    BREAK samburi.
*** Updating into DB table
    DELETE FROM zb1_stock_new WHERE uname = sy-uname.

    MODIFY zb1_stock_new FROM TABLE gt_file.
    COMMIT WORK.
    IF sy-subrc <> 0.
...
***    REFRESH :lt_item, lt_bapiret.
    ...
    IF sy-subrc = 0 AND <ls_file>-move_type = '561' AND <ls_file>-status_flag <> 'X'.
***        UPDATE zb1_stock_new SET status_flag = c_e message = 'Record is already Posted with this Key' WHERE matnr = <ls_file>-matnr AND b1_batch = <ls_file>-b1_batch
***                              AND move_type = <ls_file>-move_type AND plant = <ls_file>-plant AND uname = sy-uname AND erdat = sy-datum.
****        COMMIT WORK.
      MOVE-CORRESPONDING <ls_file> TO gw_msg.
      gw_msg-msg = 'Entry Exists in ZB1_S4_MAP Table'.    "Message Log.
...
*--> Prepare_B1_Data -> sjena <- 12.02.2020 3:16:37 PM
    IF <ls_file>-move_type = '561' AND <ls_file>-status_flag <> 'X'.    "Initial Stock
      ...
      APPEND ls_b1_s4_map TO lt_b1_s4_map01.
      CLEAR : ls_b1_s4_map.
    ELSEIF <ls_file>-move_type = '561' AND <ls_file>-status_flag = 'X'. "Returns
*--> Uploads with return indicator will not captured for update -> sjena <- 26.03.2020 12:18:03
***      READ TABLE lt_b1_s4_map WITH KEY matnr = <ls_file>-matnr plant = <ls_file>-plant b1_batch = <ls_file>-b1_batch TRANSPORTING NO FIELDS BINARY SEARCH.
...
***      ENDIF.
    ENDIF.
*--> Prepare Spool Log -> sjena <- 12.02.2020 3:29:07 PM
    MOVE-CORRESPONDING <ls_file> TO gw_msg.
    APPEND gw_msg TO gt_msg01.

*--> Call BAPI when 400 Line items reached -> sjena <- 12.02.2020 3:12:07 PM
...
***          READ TABLE lt_b1_s4_map WITH KEY matnr = <ls_file>-matnr plant = <ls_file>-plant b1_batch = <ls_file>-b1_batch TRANSPORTING NO FIELDS BINARY SEARCH.
***          IF sy-subrc IS NOT INITIAL.
          IF lt_b1_s4_map01[] IS NOT INITIAL.
            INSERT zb1_s4_map FROM TABLE lt_b1_s4_map01.
          ENDIF.
***          ENDIF.
          COMMIT WORK.
          REFRESH : lt_b1_s4_map01.
        ENDIF.
****        MOVE-CORRESPONDING <ls_file> TO gw_msg.
        CLEAR : gw_msg.
...
***        CLEAR : gw_msg.
      ELSE.
        REFRESH : lt_b1_s4_map01.   "Refresh As doc.failed
