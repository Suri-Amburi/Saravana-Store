...
IF git_file IS NOT INITIAL.
 LOOP AT  git_file ASSIGNING FIELD-SYMBOL(<fs>).
    ...
    wa_addf-s4_batch   = <fs>-s4_batch.
    wa_addf-matnr      = <fs>-matnr.
    wa_addf-plant      = <fs>-werks.
    wa_addf-b1_batch   = <fs>-b1_batch.
    wa_addf-b1_vendor  = <fs>-b1_vendor.
    wa_addf-amount     = <fs>-amount.

   MODIFY zb1_s4_map FROM wa_addf.
   CLEAR wa_addf.
