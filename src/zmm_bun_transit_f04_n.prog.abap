...
*    DATA(LV_DATE) =  SY-DATUM - 15.
  ...
  SELECT
       ...
       INTO TABLE @GT_ITEM
       WHERE ZINW_T_HDR~STATUS = @C_STATUS AND ZINW_T_ITEM~MATKL IN @S_MATKL AND
             ZINW_T_HDR~ERDATE IN @R_DATE  AND MARA~ZZPRICE_FRM IN @S_RANGE AND MARA~ZZPRICE_TO IN @S_RANGE.
  BREAK SAMBURI.
...
*** GET MARCHNDISE CAT BY MATERIAL GROUP
    ...
    IF S_MATKL IS INITIAL.
      LOOP AT S_MATKL ASSIGNING FIELD-SYMBOL(<LS_MATKL>).
        APPEND VALUE #( LOW = <LS_MATKL>-LOW HIGH = <LS_MATKL>-HIGH SIGN = <LS_MATKL>-SIGN OPTION = <LS_MATKL>-OPTION ) TO R_MATKL.
      ENDLOOP.
    ELSE.
  ...
  LOOP AT GT_KLAH_I ASSIGNING <GS_KLAH_I>.
    ...
    LOOP AT GT_KLAH_H ASSIGNING <GS_KLAH_H> FROM SY-TABIX.
      ...
      LOOP AT GT_ITEM ASSIGNING <GS_ITEM> FROM SY-TABIX.
        ...
        ADD <GS_ITEM>-NETWR_P TO GS_FINAL1-PUR_TOTAL.
...
FORM TOP_OF_PAGE1 USING TOP TYPE REF TO CL_DD_DOCUMENT.
  DATA: LV_TOP1 TYPE SDYDO_TEXT_ELEMENT,
        ...
        LV_CAT1 TYPE SDYDO_TEXT_ELEMENT,
        LV_FROM TYPE SDYDO_TEXT_ELEMENT,
        LV_TO   TYPE SDYDO_TEXT_ELEMENT.

  LV_TOP1 = 'Category Code' .

  CONCATENATE LV_TOP1 S_MATKL-LOW INTO LV_CAT1 SEPARATED BY '-' .
...
*  IF S_RANGE IS NOT INITIAL .
  ...
  LV_TO = S_RANGE-HIGH .
  CONDENSE LV_TO .
  CONCATENATE 'To Price' LV_TO INTO LV_TO SEPARATED BY '-' .
  CALL METHOD TOP->ADD_TEXT
    EXPORTING
      TEXT      = LV_TO
      SAP_STYLE = 'HEADING'.
...
**** Dispalying ALV Report
    CALL FUNCTION 'REUSE_ALV_GRID_DISPLAY'
      EXPORTING
        ...
        IT_FIELDCAT                 = LT_FIELDCAT      " Field catalog with field descriptions
        I_DEFAULT                   = 'A'              " Initial variant active/inactive logic
...
*********ADDED BY BHAVANI************
  ...
  DATA : OK_9002 TYPE SY-UCOMM,
         BACK    TYPE SY-UCOMM   VALUE 'BACK1',
         MAIL    TYPE SY-UCOMM    VALUE 'MAIL',
         CANCEL  TYPE SY-UCOMM  VALUE 'CANCEL1'.
...
*          ENDIF.
          ...
          CLEAR : GS_QR_MAIL .
*          ELSE .
*            MESSAGE 'Mail is already send for this Purchase Order' TYPE 'I' DISPLAY LIKE 'E' .
*            EXIT .
...
*-------- create persistent send request ------------------------
              ...
              CLEAR LS_MAIN_TEXT.
              LS_MAIN_TEXT = 'Sub: Details of the bundle in the Transit'.
              APPEND LS_MAIN_TEXT TO MAIN_TEXT.
          ...
          CONCATENATE 'Bundles In Transit' '.pdf' INTO LV_DOC_SUBJECT.
          TRY .
...
FORM TOP_OF_PAGE USING TOP TYPE REF TO CL_DD_DOCUMENT.
  ...
  IF  S_MATKL IS NOT INITIAL.
    ...
    LV_TOP = 'Category Code' .
    CONCATENATE LV_TOP S_MATKL-LOW INTO LV_CAT SEPARATED BY '-' .
