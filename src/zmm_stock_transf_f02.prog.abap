...
********************************************************************************
* SELECT matnr,ean11 FROM mara INTO TABLE @DATA(it_mara) FOR ALL ENTRIES IN @gt_file02[] WHERE ean11 = @gt_file02-matnr AND mstae = ' ' AND attyp <> '01'.
* SELECT s4_batch,matnr,b1_batch FROM zb1_s4_map INTO TABLE @DATA(it_batch) FOR ALL ENTRIES IN @gt_file01[] WHERE b1_batch  = @gt_file01-batch
...
*           SELECT SINGLE matnr FROM mchb INTO  ls_item-material WHERE charg =  <ls_file>-batch.
*       ENDIF.
*    ENDIF.
