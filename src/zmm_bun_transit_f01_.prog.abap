...
*       INNER JOIN EKET AS EKET ON EKET~EBELN = ZINW_T_HDR~EBELN
*       INNER JOIN EKKO AS EKKO ON EKKO~EBELN = ZINW_T_HDR~EBELN
       ...
       WHERE ZINW_T_HDR~STATUS = @C_STATUS AND ZINW_T_HDR~LIFNR IN @S_LIFNR AND
             ZINW_T_HDR~ERDATE IN @R_DATE
             AND ZINW_T_HDR~INWD_DOC <> ' '."AND ZINW_T_ITEM~MATKL IN ( '91' , '81' ).
...
*  GS_FIELDCAT-SELTEXT_L      = 'Purchase Value W/O Tax'.
  GS_FIELDCAT-SELTEXT_L      = 'Purchase Value With Tax'.
...
*    EKKO~AEDAT ,
*    EKKO~EBELN ,
*    EKET~EINDT FROM EKKO AS EKKO
*   INNER JOIN EKET AS EKET ON EKKO~EBELN  = EKET~EBELN
*   FOR ALL ENTRIES IN @LT_ITEM_GRP
*   WHERE  EKKO~EBELN = @LT_ITEM_GRP-EBELN
*   INTO TABLE @DATA(GT_DATE).
...
*    READ TABLE GT_DATE ASSIGNING FIELD-SYMBOL(<LS_DATE>) WITH KEY EBELN = <LS_ITEM_QR>-EBELN.
*    IF SY-SUBRC = 0 .
********ADDED BY BHAVANI*********
...
*    READ TABLE T_FINAL ASSIGNING FIELD-SYMBOL(<L_FINAL>) WITH KEY QR_CODE = GS_FINAL2-QR_CODE .
*    IF SY-SUBRC = 0.
*      GS_FINAL2-SEL = 'X' .
*    ENDIF.
*    APPEND GS_FINAL2 TO GT_FINAL2 .
