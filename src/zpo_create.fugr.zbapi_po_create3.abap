...
*BREAK breddy.
    IF LV_EBELN IS NOT INITIAL.
      SELECT EKKO~EBELN  EKKO~BUKRS EKKO~AEDAT EKKO~BEDAT FROM EKKO  INTO  CORRESPONDING FIELDS OF TABLE IT_EKKO
        WHERE EBELN = LV_EBELN .
    ENDIF.

    IF IT_EKKO IS NOT INITIAL.
      SELECT  EKPO~EBELN  EKPO~EBELP  EKPO~WERKS FROM EKPO INTO CORRESPONDING FIELDS OF TABLE  IT_EKPO WHERE EBELN = LV_EBELN.
    ENDIF.
...
*    ENDIF.
    ...
    SELECT MARA~MATNR  MARA~ZZPO_ORDER_TXT FROM MARA INTO CORRESPONDING FIELDS OF TABLE IT_MARA FOR ALL ENTRIES IN PO_ITEM WHERE MATNR = PO_ITEM-MATNR .
...
*      WHERE EBELN = IT_EKKO-EBELN.
    ...
    WA_POHEADER-AEDAT =  WA_EKKO-AEDAT  .                                 ""HEADER-CREAT_DATE.
...
*        .
      ...
      CONDENSE WA_PO_ITEM-EBELP NO-GAPS.
      WA_POITEM-EBELP =  WA_PO_ITEM-EBELP.

*      SPLIT WA_PO_ITEM-MENGE AT '.' INTO LV_A LV_B.
      WA_POITEM-MENGE = WA_PO_ITEM-MENGE.
      WA_POITEM-NETPR = WA_PO_ITEM-NETPR.
      WA_POITEM-MATNR = WA_PO_ITEM-MATNR.
      WA_POITEM-REMARKS = WA_PO_ITEM-REMARKS.

      WA_POITEM-NETAMT  = WA_PO_ITEM-NETPR * WA_PO_ITEM-MENGE.
      WA_POHEADER-TOTAL  = WA_POHEADER-TOTAL + WA_POITEM-NETAMT .


      READ TABLE IT_MARA INTO WA_MARA WITH  KEY MATNR = WA_PO_ITEM-MATNR .
      IF WA_MARA-ZZPO_ORDER_TXT IS NOT INITIAL.
...
*      LS_MAIN_TEXT =  | 'PURCHASE ORDER NO'  : { LV_EBELN }| .
        ...
        LS_TEXT2 =  | PO. APPROVED DATE  : { WA_POHEADER-AEDAT  } | .
        CLEAR LS_MAIN_TEXT.
